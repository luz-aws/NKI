            ...
            for kernel_size in kernel_size_list:
                for batch_size in batch_size_list:
                    for image_dims in image_dims_list:
                        ...
                        #print(X)
                        #
                        #X_patched = im2col(X, kernel_size, stride=1)
                        #print(f"X_patched:{X_patched.shape}")
...
def im2col(input_data, kernel_size, stride=1):
    ...
    col = np.zeros((batch_size, input_channels, out_height*out_width, kernel_size * kernel_size))
    
    # For each batch
    for b in range(batch_size):
        for c in range(input_channels):
            # Get patches for current batch and channel
            patches = np.lib.stride_tricks.sliding_window_view(input_data[b, c], (kernel_size, kernel_size))
            # Reshape patches into the desired format
            #print(f"patches: {patches.shape}")
            #print(patches)

            patches_resize = patches.reshape(out_height * out_width, -1)
            #print(f"patches_resize:\n{patches_resize}")
            #print(patches_resize.shape)
            
            col[b, c] = patches_resize
...
def test_performance_conv2d_kernel(
    ...
    bench_func = nki.benchmark(
        warmup=5, iters=20, save_neff_name=f"file_pool_{pool_size}_{dtype_str}.neff"
    )(kernel)
